...
static void ggml_vk_init_tq_params(ggml_backend_vk_context* ctx, int head_dim) {
    if (!ctx || !ctx->device) {
        return;
    }

    if (ctx->tq_params.initialized && ctx->tq_params.head_dim == head_dim) {
        return;
    }

    if (ctx->tq_params.initialized) {
        ggml_vk_destroy_buffer(ctx->tq_params.sign1);
        ggml_vk_destroy_buffer(ctx->tq_params.sign2);
        ggml_vk_destroy_buffer(ctx->tq_params.qjl_proj);
        ctx->tq_params.initialized = false;
        ctx->tq_params.head_dim = 0;
    }

    const size_t sign_size = head_dim * sizeof(float);
    const size_t qjl_size = (size_t)head_dim * head_dim * sizeof(float);

    ctx->tq_params.sign1 = ggml_vk_create_buffer_device(ctx->device, sign_size);
    ctx->tq_params.sign2 = ggml_vk_create_buffer_device(ctx->device, sign_size);
    ctx->tq_params.qjl_proj = ggml_vk_create_buffer_device(ctx->device, qjl_size);

    if (!ctx->tq_params.sign1 || !ctx->tq_params.sign2 || !ctx->tq_params.qjl_proj) {
        VK_LOG_DEBUG("ggml_vk_init_tq_params: Failed to allocate TQ params buffers");
        ctx->tq_params.initialized = false;
        return;
    }

    std::vector<float> h_sign1(head_dim), h_sign2(head_dim);
    std::vector<float> h_qjl((size_t)head_dim * head_dim);

    std::mt19937_64 rng_rot1(TQ_SEED_ROTATION ^ 0x1);
    std::uniform_int_distribution<int> dist01(0, 1);
    for (int i = 0; i < head_dim; i++) {
        h_sign1[i] = dist01(rng_rot1) ? 1.0f : -1.0f;
    }

    std::mt19937_64 rng_rot2(TQ_SEED_ROTATION ^ 0x2);
    for (int i = 0; i < head_dim; i++) {
        h_sign2[i] = dist01(rng_rot2) ? 1.0f : -1.0f;
    }

    std::mt19937_64 rng_qjl(TQ_SEED_QJL ^ 0xA);
    std::normal_distribution<float> dist_normal(0.0f, 1.0f);
    for (size_t i = 0; i < h_qjl.size(); i++) {
        h_qjl[i] = dist_normal(rng_qjl);
        float norm2 = 0.0f;
        for (size_t j = 0; j < (size_t)head_dim; j++) {
            norm2 += h_qjl[(i / head_dim) * head_dim + j] * h_qjl[(i / head_dim) * head_dim + j];
        }
        float inv_norm = (norm2 > 0.0f) ? 1.0f / sqrtf(norm2) : 1.0f;
        for (size_t j = 0; j < (size_t)head_dim; j++) {
            h_qjl[(i / head_dim) * head_dim + j] *= inv_norm;
        }
    }

    VK_LOG_MEMORY("TQ params: sign1=" << sign_size << ", sign2=" << sign_size << ", qjl_proj=" << qjl_size);

    void* sign1_ptr = ctx->tq_params.sign1->ptr;
    void* sign2_ptr = ctx->tq_params.sign2->ptr;
    void* qjl_ptr = ctx->tq_params.qjl_proj->ptr;

    memcpy(sign1_ptr, h_sign1.data(), sign_size);
    memcpy(sign2_ptr, h_sign2.data(), sign_size);
    memcpy(qjl_ptr, h_qjl.data(), qjl_size);

    ctx->tq_params.initialized = true;
    ctx->tq_params.head_dim = head_dim;

    VK_LOG_DEBUG("ggml_vk_init_tq_params(head_dim=" << head_dim << ")");
}
...
#undef CREATE_MM
    ...
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_TQ4_0],  "dequant_q4_0",   dequant_q4_0_len,   dequant_q4_0_data,   "main", 2, 5 * sizeof(uint32_t), {256 * 16, 1, 1}, {}, 1);
...
static void ggml_vk_flash_attn(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * q, const ggml_tensor * k, const ggml_tensor * v, const ggml_tensor * mask, const ggml_tensor * sinks, ggml_tensor * dst) {
    ...
    if (k->type == GGML_TYPE_TQ4_0 && ctx->device) {
        ggml_vk_init_tq_params(ctx, HSK);
    }
    ...
    if (split_k > 1) {
        ...
        if (k->type == GGML_TYPE_TQ4_0 && ctx->tq_params.initialized) {
            vk_subbuffer sign1_buf = ggml_vk_subbuffer(ctx, ctx->tq_params.sign1, 0);
            vk_subbuffer sign2_buf = ggml_vk_subbuffer(ctx, ctx->tq_params.sign2, 0);
            vk_subbuffer qjl_proj_buf = ggml_vk_subbuffer(ctx, ctx->tq_params.qjl_proj, 0);
            ggml_vk_dispatch_pipeline(ctx, subctx, pipeline,
                                        {q_buf, k_buf, v_buf, mask_buf, sinks_buf, split_k_buf, mask_opt_buf,
                                         sign1_buf, sign2_buf, qjl_proj_buf},
